...
CREATE TABLE public.seller_themes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Theme versions for rollback functionality
CREATE TABLE public.theme_versions (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  UNIQUE(seller_theme_id, version_number)
);

-- Hosting metrics
CREATE TABLE public.hosting_metrics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  seller_theme_id UUID REFERENCES public.seller_themes(id) ON DELETE CASCADE,
  date DATE NOT NULL,
...
CREATE POLICY "Sellers can manage their theme versions" ON public.theme_versions
  FOR ALL USING (EXISTS (
    SELECT 1 FROM public.seller_themes 
    WHERE id = theme_versions.seller_theme_id AND seller_id = auth.uid()
  ));
